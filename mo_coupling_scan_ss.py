...
'''
3. collect data every ss points along each axis
'''
ss = [1,2,5]
for m in ss:
    indice = []
    ...
    for j in range(0, 41):
        for k in range(0, 41):
            for l in range(0, 41):
                if  (j%m == 0) and (k%m == 0) and (l%m ==  0):
                    indice.append(j*41*41 +k*41+l)
    ...
    for ii, i in enumerate(indice):
        new_c_homo.append(c_homo[i])
        new_c_lumo.append(c_lumo[i])
        new_homo_pairs.append(homo_pairs[i])
        new_lumo_pairs.append(lumo_pairs[i])
